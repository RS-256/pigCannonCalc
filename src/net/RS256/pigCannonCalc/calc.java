...
public class calc {
    ...
    public static int[] estimateTarget(int[] tick, int initialX, int initialZ, double minDistance){

        int[] estimatedCoordinate = new int[2];

        estimatedCoordinate[0] = (int) Math.round(initialX + (double) (tick[0] + tick[1]) * minDistance);
        estimatedCoordinate[1] = (int) Math.round(initialZ + (double) (tick[0] - tick[1]) * minDistance);
